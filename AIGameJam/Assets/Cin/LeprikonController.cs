...
public class LeprikonController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Coroutine'leri başlat
        StartCoroutine(FireLazerAtRandomIntervals());
        StartCoroutine(FireCloudBulletAtRandomIntervals());
        StartCoroutine(TeleportAtRandomIntervals()); // Yeni coroutine'i başlat
    }
    ...
    void Die()
    {
        ...
        Debug.Log(gameObject.name + " isimli Leprikon öldü.");
    ...
    IEnumerator FireLazerAtRandomIntervals()
    {
        while (currentHealth > 0)
        {
            ...
            if (targetDistance <= triggerDistance)
            {
                ...
                GameObject instantiatedLazer = Instantiate(lazer, spawnPosition, Quaternion.identity);
    ...
    IEnumerator FireCloudBulletAtRandomIntervals()
    {
        while (currentHealth > 0)
        {
            ...
            if (targetDistance <= triggerDistance)
            {
                ...
                GameObject instantiatedCloudBullet = Instantiate(cloudBullet, spawnPosition, Quaternion.identity);
                Rigidbody2D cloudBulletRb = instantiatedCloudBullet.GetComponent<Rigidbody2D>();
                cloudBulletRb.velocity = new Vector2(direction.x, 0) * speed * 3;
            }
        }
    }

    IEnumerator TeleportAtRandomIntervals()
    {
        while (currentHealth > 0)
        {
            yield return new WaitForSeconds(Random.Range(2f, 6f));

            // Yeni rastgele x pozisyonunu ayarla
            Vector3 newPosition = transform.position;
            newPosition.x = Random.Range(-10f, 10f);

            // Düşmanı yeni pozisyona taşı
            transform.position = newPosition;

            Instantiate(teleportation, transform.position, Quaternion.identity);
        }
    }
